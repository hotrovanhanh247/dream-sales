  ...
  try {
    ...
    const dashboard = await getCachedOrCompute(cacheKey, 300, async () => {
      ...
      const [currentMonthTxns, lastMonthTxns, totalCustomers, teamSize] = await Promise.all([
        ...
        prisma.transaction.findMany({
          where: { ctvId: userId, channel: 'ctv', createdAt: { gte: startOfLastMonth, lt: startOfMonth }, status: 'CONFIRMED' },
          select: { totalAmount: true },
        }),
...
async function computeKpi(userId, monthStr, rank) {
  ...
  if (rank === 'TP') maintenance.requirements.push({ label: 'Combo nhóm', current: branchCombos, target: 150 });
  else if (rank === 'GDV') maintenance.requirements.push({ label: 'Combo nhóm', current: branchCombos, target: 550 });
  else if (rank === 'GDKD') maintenance.requirements.push({ label: 'Combo nhóm', current: branchCombos, target: 2000 });
  if (rank !== 'CTV') {
  ...
  let promotion = null;
  if (rank === 'CTV') {
    promotion = { targetRank: 'PP', requirements: [{ label: 'Combo cá nhân', current: personalCombos, target: 50 }] };
  } else if (rank === 'PP') {
...
// Org tree — selfCombos + teamCombos per node, depth-capped at 4 below the requester.
...
// Management fees received (F1/F2/F3)
router.get('/management-fees', validate(schemas.monthQuery, 'query'), async (req, res) => {
  try {
    ...
    res.json({ month, summary: { f1: summary.f1, f2: summary.f2, f3: summary.f3, total: summary.total }, records: detailed });
  } catch (err) {
    console.error('[ctv]', err); res.status(500).json({ error: 'Internal server error' });
  }
});

// Breakaway fees received (phase 1)
router.get('/breakaway-fees', validate(schemas.monthQuery, 'query'), async (req, res) => {
  try {
    ...
    if (everReceived === 0) {
      return res.json({ month, eligible: false, summary: { level1: 0, level2: 0, level3: 0, total: 0 }, records: [] });
    }
    ...
    res.json({
      month,
      eligible: true,
      summary: { level1: summary.level1, level2: summary.level2, level3: summary.level3, total: summary.total },
      records: detailed,
    });
  } catch (err) {
    console.error('[ctv]', err); res.status(500).json({ error: 'Internal server error' });
